...
const SERVICES = [
  { title: 'Source from Industry Hubs',  icon: Archive,      img: 'https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?w=400&q=80&fit=crop&auto=format' },
  { title: 'Customize Your Products',    icon: ShoppingCart, img: 'https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?w=400&q=80&fit=crop&auto=format' },
  { title: 'Fast, reliable shipping',    icon: Zap,          img: 'https://images.unsplash.com/photo-1566576721346-d4a3b4eaad5b?w=400&q=80&fit=crop&auto=format' },
  { title: 'Product monitoring',         icon: ShieldCheck,  img: 'https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=400&q=80&fit=crop&auto=format' },
];
    ...
    <Link to={`/product/${product._id}`}
      ...
      <div style={{ position: 'relative', marginBottom: 12, overflow: 'hidden', borderRadius: 8,
        ...
        <img src={product.image} alt={product.name}
          ...
          onError={onImgError} />
        {discount && (
          <span style={{ position: 'absolute', top: 8, left: 8, background: '#f97316', color: '#fff',
      ...
      <main style={{ maxWidth: 1240, margin: '0 auto', padding: '24px 16px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <section style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 16, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
          <div style={{ display: 'flex', minHeight: 380 }}>
            ...
            <div style={{ width: 196, flexShrink: 0, borderRight: '1px solid #f0f0f0' }}
              ...
              <div style={{ padding: '8px 0' }}>
                {CATEGORIES.map((cat, i) => (
                  <Link key={i} to={`/products?category=${encodeURIComponent(cat)}`}
                    ...
                    onMouseEnter={e => { if (i !== 1) { e.currentTarget.style.background = '#f9fafb'; e.currentTarget.style.color = '#2563eb'; } }}
            ...
            <div style={{ flex: 1, position: 'relative', overflow: 'hidden', minHeight: 280 }}>
              ...
              <div style={{ position: 'absolute', inset: 0,
                ...
                <Link to="/products"
                  style={{ display: 'inline-block', background: '#fff', color: '#111', fontWeight: 600,
                    fontSize: 13, padding: '10px 24px', borderRadius: 8, width: 'fit-content',
            ...
            {/* User widget */}
            <div style={{ width: 204, flexShrink: 0, borderLeft: '1px solid #f0f0f0', padding: 16,
              display: 'flex', flexDirection: 'column', gap: 12 }}
                 className="hidden xl:flex">
              {/* Auth card */}
              <div style={{ background: '#eff6ff', borderRadius: 12, padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 40, height: 40, background: '#cbd5e1', borderRadius: '50%',
                ...
                {user ? (
                        <button onClick={logout}
                          style={{ background: '#ef4444', color: '#fff', textAlign: 'center', padding: '9px 8px',
                            borderRadius: 8, fontSize: 12, fontWeight: 600, border: 'none', cursor: 'pointer', width: '100%' }}>
                          Log out
                        </button>
                      ) : (
                        <>
                          <Link to="/register"
                            style={{ background: '#2563eb', color: '#fff', textAlign: 'center', padding: '9px 8px',
                              borderRadius: 8, fontSize: 12, fontWeight: 600, display: 'block', textDecoration: 'none' }}>
                            Join now
                          </Link>
                          <Link to="/login"
                            style={{ background: '#fff', color: '#444', textAlign: 'center', padding: '9px 8px',
                              borderRadius: 8, fontSize: 12, fontWeight: 600, border: '1px solid #e5e7eb',
        ...
        <section style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 16, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
          <div style={{ display: 'flex', minHeight: 160 }}>
            ...
            {/* Countdown */}
            <div style={{ width: 200, flexShrink: 0, padding: '24px 20px', borderRight: '1px solid #f0f0f0',
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {[
                  ...
                  <div key={i} style={{ flex: 1, background: '#1f2937', borderRadius: 8, padding: '8px 4px',
                    ...
                    <span style={{ color: 'rgba(255,255,255,0.45)', fontSize: 8, fontWeight: 700,
                      marginTop: 3, letterSpacing: '0.05em' }}>{t.l}</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div style={{ display: 'flex', height: '100%' }}>
                {DEALS.map((item, i) => (
                  <Link key={i} to="/products"
                    ...
                    <span style={{ fontSize: 12, fontWeight: 600, color: '#333', marginBottom: 8, lineHeight: 1.3 }}>
                      {item.name}
                    </span>
          ...
          <div style={{ display: 'flex', minHeight: 220 }}>
            ...
            <div style={{ width: 200, flexShrink: 0, position: 'relative' }} className="hidden sm:block">
              <img
                ...
                onError={onImgError}
              />
              <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.4)', padding: 20,
                display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <h3 style={{ color: '#fff', fontSize: 17, fontWeight: 800, lineHeight: 1.3 }}>Home and<br />interiors</h3>
                <Link to="/products"
                  style={{ background: '#fff', color: '#222', fontSize: 11, fontWeight: 600,
                    padding: '6px 14px', borderRadius: 6, display: 'inline-block', textDecoration: 'none',
                    width: 'fit-content' }}>
                  Source now
                </Link>
              </div>
          ...
          {loading ? (
            ...
            <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 16, padding: '64px 24px', textAlign: 'center' }}>
              ...
              <p style={{ color: '#6b7280', fontSize: 13, maxWidth: 280, margin: '0 auto' }}>
                {error
                  ? 'Could not connect to the server. Start the backend and refresh.'
                  : 'Products will appear here once added.'}
              </p>
              {error && (
                <button onClick={() => window.location.reload()}
                  style={{ marginTop: 20, background: '#2563eb', color: '#fff', fontWeight: 600,
                    fontSize: 13, padding: '10px 24px', borderRadius: 8, border: 'none', cursor: 'pointer' }}>
                  Retry
                </button>
              )}
            </div>
        ...
        {/* ═══ CTA / INQUIRY ═══ */}
        <section style={{ background: 'linear-gradient(135deg, #1a7fff 0%, #0047d4 100%)',
          borderRadius: 16, padding: 'clamp(32px, 5vw, 48px) clamp(20px, 5vw, 40px)',
          ...
          <div style={{ position: 'relative', display: 'grid',
            gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 36, alignItems: 'center' }}>
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <input type="text" placeholder="What item do you need?"
                  style={{ width: '100%', padding: '10px 14px', border: '1px solid #e5e7eb',
                    borderRadius: 10, fontSize: 13, outline: 'none', color: '#333', boxSizing: 'border-box' }} />
                <textarea placeholder="Type more details..."
                  style={{ width: '100%', padding: '10px 14px', border: '1px solid #e5e7eb',
                    borderRadius: 10, fontSize: 13, outline: 'none', height: 72, resize: 'none',
                    color: '#333', boxSizing: 'border-box' }} />
                <div style={{ display: 'flex', gap: 8 }}>
                  <input type="text" placeholder="Quantity"
                    style={{ flex: 1, padding: '10px 14px', border: '1px solid #e5e7eb',
                      borderRadius: 10, fontSize: 13, outline: 'none', minWidth: 0 }} />
                  <select style={{ flex: 1, padding: '10px 14px', border: '1px solid #e5e7eb',
                    borderRadius: 10, fontSize: 13, background: '#fff', outline: 'none', color: '#555', minWidth: 0 }}>
                    <option>Pcs</option><option>Kg</option><option>Box</option>
                  </select>
                </div>
                <button style={{ width: '100%', background: '#2563eb', color: '#fff', fontWeight: 600,
                  fontSize: 13, padding: '12px', borderRadius: 10, cursor: 'pointer', border: 'none',
        ...
        <section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 16 }}>
            {SERVICES.map((s, i) => {
              const Icon = s.icon;
              return (
                <div key={i} style={{ background: '#fff', borderRadius: 16, border: '1px solid #f0f0f0',
                  ...
                  <div style={{ padding: '18px 16px 14px', position: 'relative' }}>
                    <div style={{ position: 'absolute', top: -18, right: 16, width: 36, height: 36,
                      background: '#2563eb', borderRadius: '50%', border: '3px solid #fff',
        ...
        {/* ═══ NEWSLETTER ═══ */}
        <section style={{ background: 'linear-gradient(135deg, #f8faff 0%, #eef2ff 100%)',
          border: '1px solid #e0e7ff', borderRadius: 16, padding: 'clamp(32px, 5vw, 48px) 24px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 8, maxWidth: 420, margin: '0 auto', flexWrap: 'wrap' }}>
            <div style={{ flex: 1, minWidth: 200, position: 'relative' }}>
              <Send size={14} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: '#aaa', pointerEvents: 'none' }} />
              <input type="email" placeholder="Email address"
                style={{ width: '100%', padding: '11px 14px 11px 34px', border: '1px solid #ddd',
                  borderRadius: 10, fontSize: 13, outline: 'none', background: '#fff',
                  boxShadow: '0 1px 4px rgba(0,0,0,0.05)', boxSizing: 'border-box' }} />
            </div>
            <button style={{ background: '#2563eb', color: '#fff', fontWeight: 600, fontSize: 13,
