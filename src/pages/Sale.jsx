...
const SaleCard = React.memo(({ product }) => {
  ...
  return (
    <Link to={`/product/${product._id}`}
      className="sale-card-interactive"
  ...
  return (
    <>
      <style>{`
        ...
        .sale-card-interactive:hover { box-shadow: 0 8px 24px rgba(0,0,0,0.09) !important; border-color: #bfdbfe !important; }
        .sale-card-interactive:hover .product-img-scale { transform: scale(1.08) !important; }
        .btn-cart-sale:hover { background: #2563eb !important; color: #fff !important; }
        .nav-link-hover:hover { color: #2563eb !important; }
        .btn-page-sale:hover:not(:disabled) { background: #f9fafb !important; }
        .btn-page-sale-active:hover { background: #f9fafb !important; border-color: #fca5a5 !important; }
        .browse-btn:hover { background: #1d4ed8 !important; }
      `}</style>
      ...
      <main style={{ maxWidth: 1240, margin: '0 auto', padding: '24px 16px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <section style={{ background: 'linear-gradient(135deg, #ef4444 0%, #b91c1c 100%)', borderRadius: 16, padding: '32px 28px', position: 'relative', overflow: 'hidden', boxShadow: '0 4px 20px rgba(239,68,68,0.25)' }}>
          ...
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                ...
                <span style={{ color: 'rgba(255,255,255,0.8)', fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Limited time offers</span>
              </div>
              <h1 style={{ color: '#fff', fontWeight: 900, fontSize: 'clamp(24px, 4vw, 40px)', lineHeight: 1.1, margin: '0 0 8px' }}>Sale — up to {topDiscount || '??'}% off</h1>
              <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, margin: 0, maxWidth: 380 }}>Grab the best deals before they're gone. New items added daily.</p>
            </div>
            {!loading && (
              <div style={{ background: 'rgba(255,255,255,0.12)', borderRadius: 14, padding: '16px 24px', textAlign: 'center', backdropFilter: 'blur(4px)' }}>
                ...
                <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', margin: '4px 0 0' }}>items on sale</p>
              </div>
            )}
          </div>
        </section>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '10px 14px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: '#111' }}>{loading ? '—' : total} sale {total === 1 ? 'item' : 'items'}</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        {loading ? (
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 16, padding: '80px 24px', textAlign: 'center', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            ...
            <h3 style={{ fontSize: 16, fontWeight: 700, color: '#374151', marginBottom: 8 }}>No sale items right now</h3>
            <p style={{ color: '#9ca3af', fontSize: 13, maxWidth: 280, margin: '0 auto 20px' }}>Check back soon — new deals are added daily.</p>
            <Link to="/products" className="browse-btn" style={{ background: '#2563eb', color: '#fff', fontWeight: 600, fontSize: 13, padding: '10px 24px', borderRadius: 8, textDecoration: 'none', display: 'inline-block', transition: 'background 0.15s' }}>
              Browse all products
            </Link>
          </div>
        ) : (
        ...
        {pages > 1 && !loading && (
          <div style={{ display: 'flex', justifyContent: 'center', gap: 4, marginTop: 16 }}>
            <button className="btn-page-sale" disabled={page === 1} onClick={() => updateParam('page', page - 1)} style={{ width: 36, height: 36, borderRadius: 8, border: '1px solid #e5e7eb', background: '#fff', cursor: page === 1 ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: page === 1 ? 0.4 : 1, transition: 'background 0.15s' }}>
            ...
            {pageNums().map((p, i) =>
              p === '…'
                ...
                : <button key={p} className={page === p ? "btn-page-sale-active" : "btn-page-sale"} onClick={() => updateParam('page', p)} style={{ width: 36, height: 36, borderRadius: 8, fontWeight: 700, fontSize: 13, border: '1px solid', cursor: 'pointer', borderColor: page === p ? '#ef4444' : '#e5e7eb', background: page === p ? '#ef4444' : '#fff', color: page === p ? '#fff' : '#555', transition: 'background 0.15s, color 0.15s, border-color 0.15s' }}>
                    {p}
                  </button>
            )}

            <button className="btn-page-sale" disabled={page === pages} onClick={() => updateParam('page', page + 1)} style={{ width: 36, height: 36, borderRadius: 8, border: '1px solid #e5e7eb', background: '#fff', cursor: page === pages ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: page === pages ? 0.4 : 1, transition: 'background 0.15s' }}>
